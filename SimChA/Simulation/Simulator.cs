...
public class Simulator
{
    ...
    public void SampleEvents(Sample sample, bool useMCMC)
    {
        ...
        _counter = 0;
        var (root, childLoopUp) = CloneIn.CreateLookUp(sample.Clones);
        sample.Kars[root.CloneId] = new Karyotype(sample.SexXX);
        if (useMCMC) 
        {
            MCSampleCNEventsRec(sample, root, childLoopUp);
        }
        else
        {
            ApplyCNEventsRec(sample, root, childLoopUp);
        }
    }

    private void ApplyCNEventsRec(Sample sample, CloneIn node, Dictionary<int, List<CloneIn>> clones)
    {
        foreach (var child in clones[node.CloneId])
        {
            ...
            for (var mutNo = 0; mutNo < child.Distance; mutNo++)
            {
                ...
                var abberation = new CNEventDesc(child.CloneId, eventP.Type, eventData.ToString(), dFit, newFitness);
                sample.EventDescs.Add(abberation);
                oldFitness = newFitness;
            }
            _counter++;
            if (child.CloneId != node.CloneId)
            {
                ApplyCNEventsRec(sample, child, clones);
            }
        }
    }
    ...
    public static List<Sample> SamplesFromProfiles(Dictionary<string, Karyotype> profiles)
    {
        ...
        {
            var clone = new CloneIn(0, -1, 0,  0);
            var sample = new Sample(profile.Key, profile.Value.SexXX, new List<CloneIn>{ clone }, new List<CNEventPars>());
            samples.Add(sample);
        }
        return samples;
    }
    ...
    public double Potential(Karyotype kar, double targetFit, List<BaseEventData> events, ref bool thresholdAccept)
    {
        ...
        {
            events[i].ApplyEvent(kar);
            eventPotentialTotal *= events[i].CNEventPars.Prob;
        }

        double newFitness = kar.UpdateFitness(_geneLists, _fitness);
        double dFit = newFitness - targetFit;
        thresholdAccept = Math.Abs(dFit / targetFit) < _mcParams.ThresholdFit;
    ...
    private void MCSampleCNEventsRec(Sample sample, CloneIn node, Dictionary<int, List<CloneIn>> clones)
    {
        foreach (var child in clones[node.CloneId])
        {
            var childKar = new Karyotype(sample.Kars[node.CloneId]);
            ...
            double currentPotential = Potential(new Karyotype(childKar), child.FitnessTarget, currentEventProps, ref thresholdAccept);
            ...
            for (int i = 0; i < _mcParams.NumSamplesTotal; i++)
            {
                ...
                double proposalPotential = Potential(new Karyotype(childKar), child.FitnessTarget, proposedEventProps, ref thresholdAccept);
            ...
            for (int mutNo = 0; mutNo < currentEventProps.Count; mutNo++)
            {
                ...
                double newFitness = childKar.UpdateFitness(_geneLists, _fitness);
                double dFit = newFitness - oldFitness;
                var abberation = new CNEventDesc(child.CloneId, eventData.EventType, eventData.ToString(), dFit, newFitness);
                sample.EventDescs.Add(abberation);
            }
            _counter++;            
            if (child.CloneId != node.CloneId)
            {
                MCSampleCNEventsRec(sample, child, clones);
            }
        }
    }
}
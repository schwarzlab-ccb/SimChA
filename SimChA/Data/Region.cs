...
public record Region(long Start, long End, string ChrNo, bool Hap1, List<SNV>? SNVs = null) : GenRange(Start, End, ChrNo)
{
    ...
    public int NumSNVsBetween(long start, long end)
        => SNVs is { Count: > 0 } ? SNVs.Count(s => s.Location >= start && s.Location <= end) : 0;
}

public record PArm(long Start, long End, string ChrNo, bool Hap1, List<SNV>? SNVs = null) 
    : Region(Start, End, ChrNo, Hap1, SNVs)
{
    public override string ToString() => base.ToString();
}

public record QArm(long Start, long End, string ChrNo, bool Hap1, List<SNV>? SNVs = null) 
    : Region(Start, End, ChrNo, Hap1, SNVs)
{
    public override string ToString() => base.ToString();
}

public record Centromere(long Start, long End, string ChrNo, bool Hap1,List<SNV>? SNVs = null) 
    : Region(Start, End, ChrNo, Hap1, SNVs)
{ 
    public override string ToString() => base.ToString();
}
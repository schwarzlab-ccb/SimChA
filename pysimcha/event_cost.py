...
def log_like_ratios(events_ratios, obs_ratios):
    return - np.sum(obs_ratios * np.log(events_ratios))
...
def get_event_ratios(events_df):
    ...
    event_ratio["Total"] = total_gains/total_losses if total_losses != 0 else float('inf')
    return event_ratio
...
for i, subdir in enumerate(subdirs):
    ...
    ll_ratios = log_like_ratios(pd.Series(event_ratios), obs_ratios)
    
    res.append([event_cost, alpha, ll, ll_ratios])


# %%
square_fig = (7,4)
df = pd.DataFrame(res, columns=["event_cost", "alpha", "ll", "ratios"])
df.to_csv("event_cost.tsv", sep="\t", index=False)
pivot_df = df.pivot_table(index="event_cost", columns="alpha", values="ll")
...
ax.legend()
fig.savefig("../img/event_cost_long.png", dpi=300, bbox_inches="tight")
fig.savefig("../img/event_cost_long.pdf")

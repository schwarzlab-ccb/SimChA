...
# Get the chromosome number that contributes the most to a given contig 
def get_majority_chromosome(contig):
    winner = ''
    longest_length = 0
    # Loop over the regions in the contig
    for i, item in enumerate(contig):
        chr_no = item["chr"]
        # Don't care about the direction of the contig
        length = round(abs(item["end"]-item["start"]) / 1000000, 2)
        if length > longest_length:
            winner = chr_no
    return winner

# Function to group the contigs into a spectral karyotyping stain-structure
def group_contigs(contigs):
    contig_dict = {chr_no : [] for chr_no in chromosome_colors}
    for i, contig in enumerate(contigs):
        majority_chr = get_majority_chromosome(contig)
        # Try to add the majority contig to the dictionary's existing entries
        try:
            contig_dict[majority_chr].append(contig)
        except KeyError:
            contig_dict[majority_chr] = [contig]
    return contig_dict
...
def plot_karyotype(data, sample_name, dpi=150):
    ...
    grouped_contigs = group_contigs(data)
    # Counter for the drawing
    i = 0
    # Loop over the chromosome ids
    for chr_no_iter in chromosome_colors:
        print("YYYYYYYYYY")
        print(grouped_contigs[chr_no_iter])
        last_left = False
        # Loop over the contigs that are majority this chromosome
        for temp_contig in grouped_contigs[chr_no_iter]:
            print(temp_contig[0])
            # sexXX samples will have an empty Y chromosome array
            try:
                contig = temp_contig[0]
            except IndexError:
                    continue
            
            for j, item in enumerate(temp_contig):
                print(item)
                chr_no = item["chr"]
                start = item["start"]
                end = item["end"]

...
public class CmdOptions
{
    ...
    [Option('v', "variants", Required = false, Default = false, HelpText = "Write out VCF file with the variants of the final simulated karyotype. Requires data/<assembly>/genome.fa (see scripts/download_ref_data.py).")]
    public bool WriteVariants { get; set; }

    [Option('f', "fasta", Required = false, Default = false, HelpText = "Write out a FASTA file for each sample. Requires data/<assembly>/genome.fa (see scripts/download_ref_data.py). WARNING! Average file size is 6GB per sample.")]
